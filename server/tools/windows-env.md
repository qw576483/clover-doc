...
> **注意：** 工具**不会自动下载**组件。缺组件时 **`env.exe start`**（以及 `restart` / 双击无参启动）会打印官方下载地址；
> ⛔ **`info` 不打印下载地址**（它走 `svc.Info()` 的状态表，只有"组件：缺失/已就绪"这一列），别拿它找下载链接。
...
1. [快速上手](../quickstart.md) —— 运行完整 demo
2. [环境安装](../development/environment.md) —— 完整的环境安装指南
