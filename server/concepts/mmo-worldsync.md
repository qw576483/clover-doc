...
> 服务端加载（`pkg/domain/mmo/mapdata`：`Load` → `ApplyTo(scene)`，位图 → `NavGrid3`、AABB → `Collider3(GroupWall)`、出生点净空校验；文件含**命名标记点**段（flags bit2）时一并完整解析并校验（`readMarkers` → `Map.Markers`，服务端当前只校验不消费，消费方按名字自行过滤）、
